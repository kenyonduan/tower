class TodosController < ApplicationController
  ...
  before_action :find_todo, :check_todo_access, except: :create
  ...
  def check_todo_access
    if current_user != @todo.creator # 创建者就一定有权限
      render json: permission_denied_resp unless current_user.accessible_projects_id(@project.team).include?(@todo.todo_list.project_id)
    end
  end
